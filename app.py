...
# --- 2050 TITAN THEME REGISTRY (MERGED FROM EXTERNAL FILE) ---
THEME_REGISTRY = {
...
def gen_inventory():
    ...
    return f'<section id="inventory" style="background:var(--bg);"><div class="container"><div class="section-head reveal" style="text-align:center; margin-bottom:3rem;"><h2 id="store-title" style="color:var(--txt-h);">Latest Collection</h2><div style="width:60px; height:4px; background:var(--s); margin:1rem auto; border-radius:2px;"></div></div><div id="inv-grid" class="grid-3"><div>Loading...</div></div></div>{voice_btn}</section>{gen_inventory_js_client(is_demo=False)}'
...
def gen_footer():
    ...
    if li_link: icons += f'<a href="{li_link}" target="_blank" rel="noopener noreferrer" style="display:inline-block; margin-right:15px; color:white;"><svg viewBox="0 0 24 24" width="24" fill="currentColor"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2a2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6zM2 9h4v12H2zM4 2a2 2 0 1 1-2 2a2 2 0 0 1 2-2z"></path></svg></a>'
    if yt_link: icons += f'<a href="{yt_link}" target="_blank" rel="noopener noreferrer" style="display:inline-block; margin-right:15px; color:white;"><svg viewBox="0 0 24 24" width="24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.5 12 3.5 12 3.5s-7.505 0-9.377.55a3.016 3.016 0 0 0-2.122 2.136C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.55 9.376.55 9.376.55s7.505 0 9.377-.55a3.016 3.016 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg></a>'
    ...
    <footer><div class="container">
        ...
        <div style="border-top:1px solid rgba(255,255,255,0.1); margin-top:3rem; padding-top:2rem; text-align:center; color:rgba(255,255,255,0.5);">
            &copy; {datetime.datetime.now().year} {biz_name}. Powered by Titan Engine.
        </div>
    </div></footer>
    """
...
def generate_modern_css():
    ...
    :root {{
        ...
        --glass: {t['glass']}; --border: {t['border']}; --radius: {t['radius']};
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    body {{ background: var(--bg); color: var(--txt); font-family: var(--b-font); line-height: 1.8; letter-spacing: 0.01em; overflow-x: hidden; font-size: var(--p-size); transition: 0.3s; }}
    body.dark-mode {{ --bg: #0f172a; --txt: #f8fafc; --txt-h: #ffffff; --card: #1e293b; --glass: rgba(30, 41, 59, 0.9); --border: rgba(255, 255, 255, 0.1); }}
    
    h1, h2, h3, h4 {{ font-family: var(--h-font); color: var(--txt-h); line-height: 1.1; font-weight: 800; margin-bottom: 1.5rem; }}
    ...
    .card {{ background: var(--card); border-radius: var(--radius); border: var(--border); box-shadow: var(--shadow); transition: 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); display: flex; flex-direction: column; overflow: hidden; color: var(--txt) !important; {backdrop} }}
    ...
    .card-desc {{ display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; font-size:0.95rem; opacity:0.8; }}
    
    .btn {{ display: inline-flex; align-items: center; justify-content: center; padding: 1.2rem 2.5rem; border-radius: var(--radius); font-weight: 800; transition: all 0.3s ease; text-transform: uppercase; letter-spacing: 1.5px; cursor:pointer; border:none; position: relative; overflow: hidden; }}
    ...
    .mobile-menu {{ display: none; font-size: 1.8rem; cursor: pointer; color:var(--txt-h); z-index: 2001; }}
    
    .modern-hero {{ position: relative; min-height: 100vh; display: flex; align-items: center; text-align: {align}; padding-top: 120px; overflow: hidden; background: var(--p); }}
    ...
    .modern-hero-grid {{ display: grid; grid-template-columns: 1.1fr 1fr; gap: 4rem; align-items: center; width: 100%; z-index: 2; position: relative; }}
    ...
    .hero-badge {{ display: inline-block; padding: 0.5rem 1.5rem; margin-bottom: 1.5rem; background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.3); border-radius: 50px; font-size: 0.85rem; font-weight: 700; color: #fff; text-transform: uppercase; letter-spacing: 1px; }}
    .modern-hero h1 {{ color: #ffffff !important; margin-bottom: 1.5rem; text-shadow: 0 10px 30px rgba(0,0,0,0.5); }}
    .modern-hero p {{ color: rgba(255,255,255,0.9) !important; font-size: clamp(1.2rem, 2vw, 1.4rem); margin-bottom: 2.5rem; max-width: 800px; font-weight:400; }}
    ...
    .gallery-thumbs {{ display: flex; gap: 15px; overflow-x: auto; padding-bottom: 10px; }}
    ...
    .thumb:hover, .thumb.active {{ border-color: var(--p); opacity: 1; }}
...
def build_page(title, content):
    ...
    nav_html = f"""
        {top_bar}
        <nav class="glass" id="main-navbar" style="top:{nav_top_offset}">
            <div class="container nav-flex">
...
# --- 8. PREVIEW UI CONTROLS (DECLARED FIRST TO FIX NAMEERROR) ---
st.divider()
...
# --- 9. RENDER HTML CONTENT ---
html_to_render = ""

if preview_mode == "Home":
    home_content = ""
    if show_hero: home_content += gen_hero()
    if show_stats: home_content += f'<div class="stats-ribbon-container container reveal" style="margin-top:2rem;"><div class="stats-ribbon" style="display:flex; justify-content:space-around; background:var(--p); color:white; padding:2rem; border-radius:12px;"><div class="stat-block" style="text-align:center;"><h3 style="color:white; font-size:2rem; margin:0;">{stat_1}</h3><p style="margin:0; opacity:0.8;">{label_1}</p></div><div class="stat-block" style="text-align:center;"><h3 style="color:white; font-size:2rem; margin:0;">{stat_2}</h3><p style="margin:0; opacity:0.8;">{label_2}</p></div><div class="stat-block" style="text-align:center;"><h3 style="color:white; font-size:2rem; margin:0;">{stat_3}</h3><p style="margin:0; opacity:0.8;">{label_3}</p></div></div></div>'
    if show_features: home_content += gen_features()
    if show_pricing: home_content += gen_pricing_table()
    if show_inventory: home_content += f'<section id="store" style="background:var(--bg)"><div class="container"><div class="section-head reveal"><h2>Tour Packages</h2><div style="width:60px; height:4px; background:var(--s); margin:1rem auto; border-radius:2px;"></div></div><div id="store-grid" class="grid-3"></div></div></section>{gen_inventory_js_client()}'
    ...
    html_to_render = build_page("Home", home_content)

elif preview_mode == "About": html_to_render = build_page("About", f"{gen_inner_header('About Us')}<section style='background:var(--bg);'><div class='container' style='color:var(--txt);'>{format_text(about_long)}</div></section>")
    ...
    html_to_render = build_page("Contact", contact_content)
elif preview_mode == "Privacy": html_to_render = build_page("Privacy", f"{gen_inner_header('Privacy Policy')}<section style='background:var(--bg);'><div class='container' style='color:var(--txt);'>{format_text(priv_txt)}</div></section>")
elif preview_mode == "Terms": html_to_render = build_page("Terms", f"{gen_inner_header('Terms of Service')}<section style='background:var(--bg);'><div class='container' style='color:var(--txt);'>{format_text(term_txt)}</div></section>")
...
with c2:
    ...
    with zipfile.ZipFile(z_b, "a", zipfile.ZIP_DEFLATED, False) as zf:
        # We need to re-generate the home body for the zip without the st.info etc.
        z_home_body = ""
        if show_hero: z_home_body += gen_hero()
        if show_stats: z_home_body += f'<div class="stats-ribbon-container container reveal" style="margin-top:2rem;"><div class="stats-ribbon" style="display:flex; justify-content:space-around; background:var(--p); color:white; padding:2rem; border-radius:12px;"><div class="stat-block" style="text-align:center;"><h3 style="color:white; font-size:2rem; margin:0;">{stat_1}</h3><p style="margin:0; opacity:0.8;">{label_1}</p></div><div class="stat-block" style="text-align:center;"><h3 style="color:white; font-size:2rem; margin:0;">{stat_2}</h3><p style="margin:0; opacity:0.8;">{label_2}</p></div><div class="stat-block" style="text-align:center;"><h3 style="color:white; font-size:2rem; margin:0;">{stat_3}</h3><p style="margin:0; opacity:0.8;">{label_3}</p></div></div></div>'
        if show_features: z_home_body += gen_features()
        if show_pricing: z_home_body += gen_pricing_table()
        if show_inventory: z_home_body += f'<section id="store" style="background:var(--bg)"><div class="container"><div class="section-head reveal"><h2>Tour Packages</h2><div style="width:60px; height:4px; background:var(--s); margin:1rem auto; border-radius:2px;"></div></div><div id="store-grid" class="grid-3"></div></div></section>{gen_inventory_js_client()}'
        if show_gallery: z_home_body += gen_about_section()
        if show_testimonials: 
            t_cards = "".join([f'<div class="card reveal"><div class="card-body"><p style="font-size:1.1rem; font-style:italic; line-height:1.7; opacity:0.9; margin-bottom:2rem; flex-grow:1;">"{x.split("|")[1].strip()}"</p><div style="display:flex; align-items:center; gap:15px; border-top:1px solid var(--border); padding-top:1.5rem;"><div style="width:45px; height:45px; border-radius:50%; background:var(--p); color:#fff; display:flex; align-items:center; justify-content:center; font-weight:bold; font-size:1.2rem;">{x.split("|")[0].strip()[0]}</div><div><b style="color:var(--txt-h); font-size:1.1rem; display:block;">{x.split("|")[0].strip()}</b><span style="font-size:0.8rem; opacity:0.6;">Verified Client</span></div></div></div></div>' for x in testi_data.split('\n') if "|" in x])
            z_home_body += f'<section id="testimonials" style="background:rgba(128,128,128,0.02)"><div class="container"><div class="section-head reveal"><h2>Client Stories</h2><div style="width:60px; height:4px; background:var(--s); margin:1rem auto; border-radius:2px;"></div></div><div class="grid-3">{t_cards}</div></div></section>'
        if show_faq: z_home_body += gen_faq_section()
        if show_cta: z_home_body += f'<section style="background:var(--s); color:white; text-align:center;"><div class="container reveal"><h2 style="color:var(--btn-txt) !important; font-size:clamp(2.5rem, 5vw, 3.5rem); margin-bottom:1rem;">Ready to Launch?</h2><p style="margin-bottom:2rem; font-size:1.3rem; opacity:0.9; color:var(--btn-txt) !important;">Join the future of web architecture.</p><a href="contact.html" class="btn" style="background:var(--bg); color:var(--txt-h) !important;">Get Started</a></div></section>'

        zf.writestr("index.html", build_page("Home", z_home_body))
        zf.writestr("about.html", build_page("About", f"{gen_inner_header('About Us')}<section style='background:var(--bg);'><div class='container' style='color:var(--txt);'>{format_text(about_long)}</div></section>"))
        zf.writestr("contact.html", build_page("Contact", contact_content))
        zf.writestr("privacy.html", build_page("Privacy", f"{gen_inner_header('Privacy Policy')}<section style='background:var(--bg);'><div class='container' style='color:var(--txt);'>{format_text(priv_txt)}</div></section>"))
        zf.writestr("terms.html", build_page("Terms", f"{gen_inner_header('Terms of Service')}<section style='background:var(--bg);'><div class='container' style='color:var(--txt);'>{format_text(term_txt)}</div></section>"))
